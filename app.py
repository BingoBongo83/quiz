...
def index():
    ...
    if monitor_round == 9:
        return render_template('pause.html', last_question=last_question)
    if monitor_round == 8:
        player_name = procs.get_player_name(1,8)
        return render_template('end.html', player_name=player_name)
    if monitor_round == 11:
        return render_template('image.html', last_question=last_question, image=last_question['image'], round=round_id)
        print("cover")
    else:
        points = procs.show_points(round_id)
        return render_template(f"index{buzzer_blocked}.html", points=points, last_question=last_question)
